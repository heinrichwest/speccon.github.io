...
# Mass update script to add sticky banner HTML to all qualification files

BANNER_HTML='    <!-- Sticky Banner -->
...
for file in "${FILES[@]}"; do
    ...
    # Create temp file for the banner HTML
    echo "$BANNER_HTML" > temp_banner.html

    # Insert banner before footer
    if grep -q "<!-- Footer -->" "$filepath"; then
        sed -i '/<!-- Footer -->/i\'"$(cat temp_banner.html | sed 's/$/\\/')" "$filepath"
    elif grep -q "<footer" "$filepath"; then
        sed -i '/<footer/i\'"$(cat temp_banner.html | sed 's/$/\\/')" "$filepath"
    fi

    rm -f temp_banner.html
done
